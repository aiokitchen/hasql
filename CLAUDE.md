...
- **Staleness Detection:** Monitors byte (`lag["bytes"]`) or time (`lag["time"]`) replication lag and classifies stale replicas (fresh replicas > optional master fallback > known stale replica > wait)
...
| `RandomWeightedBalancerPolicy` | Preserves the legacy reflected/normalized response-time selection | Uses `pool_state.get_last_response_time()` and `random.random()` |
...
**`RandomWeightedBalancerPolicy` selection:** preserves the master algorithm:
`_reflect_times` treats `None` as zero and adds `MACHINE_EPSILON`,
`_normalize_times` transforms reflected values, and `_weighted_choice` walks
the distribution using `random.random()`. Do not replace this compatibility
behavior with `random.choices` or `_compute_weights`.
