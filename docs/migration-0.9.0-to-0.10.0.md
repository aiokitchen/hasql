...
outside grace moves the pool out of the fresh replica set. The time checker
caches fresh master WAL LSN state (default maximum age: 2 seconds), reports zero
lag when the replica replay LSN matches that state, and otherwise evaluates
replay-timestamp delay. Missing or expired master state fails open with empty
lag, consistently with the byte checker. Query errors remain fail-closed through
the health monitor, which removes the pool from availability until a later
successful check. Because time lag uses wall-clock timestamps, clock skew can
affect behind-replica values. Read acquisition prioritizes a fresh replica,
then optional master, then a stale replica, then waiting; an already-waiting
acquisition is awakened and re-evaluates when stale fallback becomes available.
Lag metrics use `lag["bytes"]` for byte checks and `lag["time"]` (a `timedelta`)
for time checks.
