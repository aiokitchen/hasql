...
class PoolState(Generic[PoolT, ConnT]):
    ...
    _read_candidate_event: asyncio.Event
    ...
    def __init__(
        ...
        self._read_candidate_event = asyncio.Event()
    ...
    async def get_replica_pools(
        ...
        while True:
            if self._replica_pool_set:
                return list(self._replica_pool_set)
            if fallback_master and self._master_pool_set:
                return list(self._master_pool_set)
            if self._stale_pool_set:
                return list(self._stale_pool_set)

            self._read_candidate_event.clear()
            if (
                self._replica_pool_set
                or (fallback_master and self._master_pool_set)
                or self._stale_pool_set
            ):
                continue
            await self._read_candidate_event.wait()
    ...
    async def confirm_master_role(self, pool: PoolT, dsn: Dsn) -> None:
        await self._add_pool_to_master_set(pool, dsn)

    def mark_dsn_ready(self, dsn: Dsn) -> None:
    ...
    async def check_replica_staleness(
        ...
        if result.is_stale:
            ...
            self._read_candidate_event.set()
        ...
        was_replica = pool in self._replica_pool_set
        self._stale_pool_set.discard(pool)
        self._last_check_result[pool] = result
        await self._add_pool_to_replica_set(pool, dsn)
        if not was_replica:
