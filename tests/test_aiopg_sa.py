...
from hasql.driver.aiopg_sa import AiopgSaDriver, PoolManager
...
def test_aiopg_sa_driver_is_independent_pool_driver():
    assert issubclass(AiopgSaDriver, PoolDriver) and not issubclass(
        AiopgSaDriver, AiopgDriver
    )


async def test_aiopg_sa_driver_releases_connection_synchronously():
    engine = MagicMock()
    connection = MagicMock()

    await AiopgSaDriver().release_to_pool(connection, engine)

    engine.release.assert_called_once_with(connection)
