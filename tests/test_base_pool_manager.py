...
async def test_release_unknown_connection_raises_value_error(dsn):
    async with TestPoolManager(
        dsn,
        refresh_timeout=0.2,
        refresh_delay=0.1,
    ) as pool_manager:
        unknown_connection = object()

        with pytest.raises(
            ValueError,
            match=(
                r"Pool\.release\(\) received invalid connection: .* "
                r"is not a member of this pool"
            ),
        ):
            await pool_manager.release(unknown_connection)


async def test_release_connection_twice_raises_value_error(dsn):
    async with TestPoolManager(
        dsn,
        refresh_timeout=0.2,
        refresh_delay=0.1,
...
async def test_terminate_closes_all_pools_and_marks_manager_closed(
    pool_manager: BasePoolManager,
):
    await pool_manager._pool_state.ready()
    pools = [
        pool for pool in pool_manager._pool_state.pools if pool is not None
    ]
    ...
    assert (
        all(
            pool_manager._pool_state.is_connection_closed(connection)
            for pool in pools
            for connection in pool.connections
        ),
        pool_manager._pool_state.master_pool_count,
        pool_manager._pool_state.replica_pool_count,
        pool_manager._closing,
        pool_manager._closed,
    ) == (True, 0, 0, False, True)


async def test_terminate_propagates_first_error_and_stops_before_later_pools(
    pool_manager: BasePoolManager,
):
    await pool_manager._pool_state.ready()
    pools = [
    ...
    async def terminate_pool(pool):
        terminated_pools.append(pool)
        if pool is pools[1]:
            raise terminate_error

    with patch.object(
        pool_manager._pool_state,
        "terminate_pool",
        AsyncMock(side_effect=terminate_pool),
    ):
    ...
    assert (
        ...
        pool_manager._closing,
        pool_manager._closed,
    ) == (terminate_error, pools[:2], 0, 0, True, False)
