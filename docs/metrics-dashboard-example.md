...
| `db.pool.acquire.count` | `host` | Cumulative successful acquire attempts |
| `db.pool.acquire.duration` | `host` | Cumulative acquire duration in seconds |
| `db.pool.stale.count` | — | Point-in-time number of stale pools |
| `db.pool.stale.status` | `host`, `role`, `staleness` | Per-pool stale flag (1 stale, 0 fresh) |
| `db.pool.stale.lag.bytes` | `host`, `role`, `staleness` | Last measured replay lag in bytes (`lag["bytes"]`) |
| `db.pool.stale.lag.time` | `host`, `role`, `staleness` | Last measured replay lag in seconds (`lag["time"]`) |
| `db.pool.extra.<key>` | `host`, `role`, `staleness` when known | Driver-specific extras (see below) |

All OTLP attributes are normalized strings. The acquire instruments are
observable cumulative counters; the stale instruments are point-in-time
gauges. `timedelta` values from the `time` lag key are converted to seconds.
