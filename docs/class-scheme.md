...
| Owns | Does NOT own |
|---|---|
| Pool creation (`pool_factory`) | Pool lifecycle operations |
...
| Pool close/terminate | Lifecycle orchestration |
...
| Adding/removing pools from role/stale sets and cached staleness results | Connection acquisition orchestration (user-facing) |
...
| `acquire()`, `release()`, readiness, and lifecycle public API | Balancer selection algorithm |
| `metrics()` — enriching raw stats into `PoolMetrics` | Health check scheduling (delegates to HealthMonitor) |
| Lifecycle: `close()` and `terminate()` | Raw pool stats (delegates to PoolState) |
| Supported state views: `available_pool_count`, `wait_masters_ready()` | Raw PoolState proxy surface |
...
| Owns | Does NOT own |
|---|---|
| Background task lifecycle (start/stop) | Role detection implementation (delegates to PoolState) |
| One task per DSN | Pool state ownership (mutates through PoolState) |
| Waiting for pool creation | Connection acquisition for user requests |
| Safe connection release on errors | |
| Role refresh followed by staleness collection/checking | Staleness policy state ownership |
| Fail-closed removal on timeout or staleness errors | |
...
| `RandomWeightedBalancerPolicy` | `random.choices` with finite, scale-equivalent, relative and uncapped inverse health-check-latency weights; uniform fallback if any timing is missing or invalid (`None`, `<= 0`, NaN, infinite); no load, query-latency, or fairness guarantee | `get_last_response_time()` |
...
For read acquisition, candidate priority is fresh replica → optional master → stale
replica → wait. If an acquisition is already waiting with no candidates, the
read-candidate event wakes it when a stale fallback becomes available so it can
