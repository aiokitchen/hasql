...
* ``hasql.balancer_policy.RandomWeightedBalancerPolicy``
  Selects with probabilities proportional to the inverse of each candidate's
  last health-check latency. Lower valid latency is more likely. If any timing
  is missing or invalid (``None``, ``<= 0``, NaN, or infinite), all candidates
  receive uniform weights. The latency is health-check latency only, not query
  latency or pool load; selection is statistical, with no deterministic, fair,
  or no-starvation guarantee.
