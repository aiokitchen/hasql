...
class TimeMockDriver:
    def __init__(
        self,
        lag_interval,
        master_lsn="0/2000000",
        replica_lsn="0/1000000",
    ):
        self._lag_interval = lag_interval
        self._master_lsn = master_lsn
        self._replica_lsn = replica_lsn

    async def fetch_scalar(self, connection, query):
        if "pg_current_wal_lsn" in query:
            return self._master_lsn
        if "pg_last_wal_replay_lsn" in query:
            return self._replica_lsn
...
@pytest.mark.asyncio
async def test_time_staleness_checker_without_master_state_fails_open():
    driver = ScalarSequenceDriver([])
    checker = TimeStalenessChecker(max_lag=timedelta(seconds=10))

    result = await checker.check(
        CheckContext(connection=MockConnection(), driver=driver),
    )
...
@pytest.mark.asyncio
async def test_time_staleness_checker_with_expired_master_state_fails_open():
    driver = ScalarSequenceDriver(["0/2000000"])
    checker = TimeStalenessChecker(
        max_lag=timedelta(seconds=10),
...
@pytest.mark.asyncio
async def test_time_staleness_checker_equal_lsn_ignores_old_transaction():
...
@pytest.mark.asyncio
async def test_time_staleness_checker_different_lsn_uses_replay_time():
    lag = timedelta(seconds=11)
    driver = ScalarSequenceDriver(["0/2000000", "0/1000000", lag])
    checker = TimeStalenessChecker(max_lag=timedelta(seconds=10))
    ctx = CheckContext(connection=MockConnection(), driver=driver)
    await checker.collect_master_state(ctx)
...
@pytest.mark.asyncio
async def test_time_staleness_checker_null_replay_time_is_stale():
    driver = ScalarSequenceDriver(["0/2000000", "0/1000000", None])
    checker = TimeStalenessChecker(max_lag=timedelta(seconds=10))
    ctx = CheckContext(connection=MockConnection(), driver=driver)
    await checker.collect_master_state(ctx)
...
@pytest.mark.asyncio
async def test_time_staleness_checker_malformed_master_lsn_is_stale():
    driver = ScalarSequenceDriver(["not-an-lsn"])
    checker = TimeStalenessChecker(max_lag=timedelta(seconds=10))
    ctx = CheckContext(connection=MockConnection(), driver=driver)
    await checker.collect_master_state(ctx)
...
@pytest.mark.asyncio
async def test_time_staleness_checker_propagates_query_exception():
    error = RuntimeError("query failed")
    driver = ScalarSequenceDriver(["0/2000000", error])
    checker = TimeStalenessChecker(max_lag=timedelta(seconds=10))
    ctx = CheckContext(connection=MockConnection(), driver=driver)
    await checker.collect_master_state(ctx)
